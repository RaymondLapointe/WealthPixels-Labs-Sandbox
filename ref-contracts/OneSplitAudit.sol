...
contract OneSplitAudit is IOneSplit, Ownable {
    ...
    IFreeFromUpTo public constant chi =
    ...
    {
        return
            oneSplitImpl.getExpectedReturnWithGas(
                fromToken,
                destToken,
                amount,
                parts,
    ...
    {
        return
            oneSplitImpl.getExpectedReturnWithGasMulti(
                tokens,
                amount,
                parts,
                flags,
    ...
    function swapWithReferralMulti(
        ...
        require(
            tokens.length >= 2 && amount > 0,
            "OneSplit: swap makes no sense"
        );
        require(
            flags.length == tokens.length - 1,
            "OneSplit: flags array length is invalid"
        );
        require(
        ...
        uint256 confirmed = tokens
            .first()
            .universalBalanceOf(address(this))
            .sub(beforeBalances.ofFromToken);
        ...
        tokens.last().universalTransfer(
            referral,
            returnAmount.mul(feePercent).div(1e18)
        );
        tokens.last().universalTransfer(
            msg.sender,
            returnAmount.sub(returnAmount.mul(feePercent).div(1e18))
        );
        ...
        if (afterBalances.ofFromToken > beforeBalances.ofFromToken) {
            tokens.first().universalTransfer(
                msg.sender,
        ...
        if (
            (flags[0] &
                (FLAG_ENABLE_CHI_BURN | FLAG_ENABLE_CHI_BURN_BY_ORIGIN)) > 0
        ) {
            uint256 gasSpent = 21000 +
                ...
                msg.data.length;
            _chiBurnOrSell(
                ((flags[0] & FLAG_ENABLE_CHI_BURN_BY_ORIGIN) > 0)
                    ? tx.origin
                    : msg.sender,
            ...
            uint256 gasSpent = 21000 +
                ...
                16 *
                msg.data.length;
            IReferralGasSponsor(referral).makeGasDiscount(
                gasSpent,
                returnAmount,
                msg.data
            );
    ...
    function _chiBurnOrSell(address payable sponsor, uint256 amount) internal {
        ...
        uint256 sellRefund = UniswapV2ExchangeLib.getReturn(
            exchange,
            chi,
            weth,
            amount
    ...
    function _getFirstAndLastBalances(IERC20[] memory tokens, bool subValue)
        internal
        view
